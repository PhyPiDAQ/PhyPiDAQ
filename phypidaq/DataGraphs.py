...
class DataGraphs(object):
    ...
    def __init__(self, ConfDict):
        ...
        if self.dT < 60:
            self.tUnit = 's'
            self.tUnitFactor = 1.0
        elif self.dT < 3600:
            self.tUnit = 'min'
            self.tUnitFactor = 1.0 / 60.0
        else:
            self.tUnit = 'h'
            self.tUnitFactor = 1.0 / 3600.0
        ...
        self.AxisLabels = [self.ChanLabels[self.Cidx0] + cu0, self.ChanLabels[self.Cidx1] + cu1]
        ...
        if self.XYmode:
            ...
            fig.subplots_adjust(left=0.2, bottom=0.1, right=0.8, top=0.94, wspace=None, hspace=0.25)
        ...
        for i in range(self.NAxes):
            ...
            axes[i].set_ylabel(self.ChanNams[Cidx] + ' ' + self.AxisLabels[i], color=self.ChanColors[Cidx])
            axes[i].grid(True, color=self.ChanColors[Cidx], linestyle='--', alpha=0.3)
        axes[0].set_xlabel('History (' + self.tUnit + ')', size='x-large')
        axes[0].axvline(0.0, color='yellow', alpha=0.5, lw=5)  # highlight t=0
        ...
        axbar[0].set_xlim(0.0, Nc)
        axbar[0].axvline(0, color=self.ChanColors[0])
        axbar[0].set_ylim(*self.ChanLim[0])
        axbar[0].axhline(0.0, color='k', linestyle='-', lw=2, alpha=0.5)
        axbar[0].set_ylabel(self.ChanNams[0] + ' ' + self.AxisLabels[0], size='x-large', color=self.ChanColors[0])
        ...
        if self.NAxes > 1:
            ...
            axbar[1].set_ylabel(
                self.ChanNams[Cidx] + ' ' + self.AxisLabels[1], size='x-large', color=self.ChanColors[Cidx]
            )
            axbar[1].grid(True, color=self.ChanColors[Cidx], linestyle='--', alpha=0.3)
        ...
        if self.XYmode:
            ...
            axXY.set_xlabel(
                self.ChanNams[cx] + ' ' + self.ChanLabels[cx] + cux, size='x-large', color=self.ChanColors[cx]
            )
            axXY.set_ylabel(
                self.ChanNams[cy] + ' ' + self.ChanLabels[cy] + cuy, size='x-large', color=self.ChanColors[cy]
            )
    ...
    def init(self):
        ...
        for i in range(self.NChan):
            ...
            (bg,) = self.axbar[iax].bar(
                self.ind[i], 0.0, self.bwidth, align='center', color=self.ChanColors[i], alpha=0.5
            )
            self.bgraphs += (bg,)
        ...
        # Values in Text form
        self.animtxt = self.axtxt.text(0.01, 0.025, ' ', size='large', transform=self.axtxt.transAxes, color='darkblue')
    ...
    def __call__(self, data):
        ...
        if data is not None:
            ...
            for i in range(self.NChan):
                ...
                txt += bgn + '%s: % #.4g%s' % (self.ChanNams[i], self.Vhist[i, k], self.ChanUnits[i]) + end
                # update bar chart
            ...
            if self.XYmode:
                ...
                for i in range(len(self.XYgraphs)):
                    ...
                    self.XYgraphs[i].set_data(self.h[cx, i1:], self.h[cy, i1:])
        # -- end if is not None
        return self.bgraphs + self.graphs + self.XYgraphs + (self.animtxt,)
